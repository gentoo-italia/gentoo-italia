...
# $Header: /var/cvsroot/gentoo-x86/dev-lang/spidermonkey/spidermonkey-1.7.0-r1.ebuild,v 1.1 2010/03/07 22:38:56 ramereth Exp $
...
src_compile() {
	...
	if use threadsafe; then
		emake -j1 -f Makefile.ref JS_HAS_FILE_OBJECT=1 LIBDIR="$(get_libdir)" JS_THREADSAFE=1 \
		|| die "emake with threadsafe enabled failed";
	else
		emake -j1 -f Makefile.ref JS_HAS_FILE_OBJECT=1 LIBDIR="$(get_libdir)" \
