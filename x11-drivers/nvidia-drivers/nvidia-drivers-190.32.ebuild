...
src_install() {
	...
	if use kernel_linux; then
		...
		newins "${WORKDIR}"/nvidia nvidia.conf || die "newins failed"
	elif use x86-fbsd; then
		insinto /boot/modules
		doins "${WORKDIR}/${NV_PACKAGE}/src/nvidia.kld" || die "doins failed"

		exeinto /boot/modules
		doexe "${WORKDIR}/${NV_PACKAGE}/src/nvidia.ko" || die "doexe failed"
	fi
	...
	# Helper Apps
	dobin ${NV_EXEC}/nvidia-xconfig || die "dobin failed"
	dobin ${NV_EXEC}/nvidia-bug-report.sh || die "dobin failed"
}
